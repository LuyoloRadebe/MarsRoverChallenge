  ...
  class Begin
    ...
    def render_game(state)
      ...
      update do
        on :mouse_down do |mouse_event|
          ...
          if @submit_button.contains?(mouse_event.x, mouse_event.y)
    ...
    def display_output(state,x)
      ...
      @output_coordinates = Text.new(
        "POSITION AND ORIENTATION: #{state.rover[x].position.x } #{state.rover[x].position.y} #{state.rover[x].orientation[0]}",
        x: 10,
        y: @output_y+ 30 + x*40,
        font: './assets/fonts/Courier_Prime/CourierPrime-Bold.ttf',
        size: 15,
